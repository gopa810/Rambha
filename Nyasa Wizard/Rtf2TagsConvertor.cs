...
namespace SlideMaker
{
    class Rtf2TagsConvertor
    {
        public string Main(string rtf)
        {
            ...
            sb.Replace(" </i>", "</i> ");
            sb.Replace(" </b>", "</b> ");
            sb.Replace("<b>Notes for parents and teachers:</b> ", "<b>Notes for parents and teachers:</b>\n");

            string[] lines = sb.ToString().Split(new char[] { '\r', '\n' });

            sb.Clear();
            foreach (string line in lines)
            {
                if (line.Contains("ChaparralPro-"))
                    continue;
                if (line.Length > 0)
                {
                    string lineTrimmed = line.TrimStart();
                    if (lineTrimmed.StartsWith("pp.") || lineTrimmed.StartsWith("p."))
                    {
                        sb.AppendLine();
                        sb.Append(line);
                    }
                    ...
                    else if (Char.IsLower(line[0]) || line[0] == '(')
                    {
                        sb.Append(" ");
                        sb.Append(line);
                    }
        ...
        public string MainOneLine(string rtf)
        {
            ...
            sb.Replace(" </i>", "</i> ");
            sb.Replace(" </b>", "</b> ");
            sb.Replace("<b>Notes for parents and teachers:</b> ", "");
