...
namespace Rambha.Script
{
    ...
    public class GSList: GSCore
    {
        ...
        public GSString Title = new GSString("");
        ...
        public GSList(IEnumerable<GSCore> list, string title)
        {
            Parts.AddRange(list);
            Title.Value = title;
        }
        ...
        public void Add(object obj)
        {
            if (obj is int)
            {
                Add(new GSInt64((int)obj));
            }
            else if (obj is double)
            {
                Add(new GSDouble((double)obj));
            }
            else if (obj is long)
            {
                Add(new GSInt64((long)obj));
            }
            else
            {
        ...
        public override GSCore GetPropertyValue(string s)
        {
            switch(s)
            {
                case "title":
                    return Title;
                case "count":
                    return new GSInt64(Parts.Count);
                default:
            ...
            foreach (char C in line)
            {
                ...
                else if (mode == 2)
                {
                    if (char.IsWhiteSpace(C) || C == ')')
                    {
                        ...
                        {
                            currentList.Add(new GSDouble() { DoubleValue = d });
                        }
                        else if (int.TryParse(value, out i))
                        {
                            currentList.Add(new GSInt64() { Int64Value = i });
                        }
                        else
                        {
